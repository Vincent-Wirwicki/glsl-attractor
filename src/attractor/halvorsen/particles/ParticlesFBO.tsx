...
import SimMatHalvorsen from "../shaders/sim/SimMat";
...
declare module "@react-three/fiber" {
  interface ThreeElements {
    simMatHalvorsen: Object3DNode<SimMatHalvorsen, typeof SimMatHalvorsen>;
  }
}

const ParticlesFBO = () => {
  const size = 512;
  const color = new Vector3(0.15, 0.03, 0.015);
  ...
  return (
    <>
      <PortalMesh uvs={uvs} positions={positions} scene={scene}>
        {/* some children */}
        <simMatHalvorsen ref={simMatRef} args={[size]} />
      </PortalMesh>
      <PointsGeo
        color={color}
